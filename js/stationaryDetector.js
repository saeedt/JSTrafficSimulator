...
stationaryDetector.prototype.update=function(time,dt,gc,stat){
  ...
  if(time>=this.iAggr*this.dtAggr+this.dtAggr){
    ...
    this.historyOcc[this.iAggr]= (this.occSum>0) ? this.occSum/(this.dtAggr*this.nLanes) : 0;
    this.time[this.iAggr] = time;
    this.optStat[this.iAggr]=stat;
    this.historyGC[this.iAggr] = (this.greenCycle.length>0) ? this.greenCycle.reduce((a,b)=>a+b,0)*1./this.greenCycle.length : 0;
    this.vehCount=0;
    this.speedSum=0;
    this.occSum=0;
    this.greenCycle=[];
...
stationaryDetector.prototype.display=function(textsize){
  ...
  +" %";
    ...
    ctx.fillStyle="rgb(0,0,0)";
    ctx.fillText(flowStr,xPixCenter-0.46*boxWidth,yPixCenter-0.25*boxHeight);
